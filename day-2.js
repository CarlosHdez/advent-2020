...
const oldRules = (input) => {
  let valid = 0
  input.forEach((line) => {
    const [_, min, max, key, pass] = line.match(regex)
    const validation = new RegExp(`(${key})`, 'g')
    if (!validation.test(pass)) {
      return
    }
    const matches = pass.match(validation).length
...
const newRules = (input) => {
  let valid = 0
  input.forEach((line) => {
    const [_, pos1, pos2, key, pass] = line.match(regex)
    const validation = new RegExp(`${key}`)
    // Word is nowhere
    if (!validation.test(pass)) {
      return
    }
    const isValid = (pass[pos1 - 1] === key && pass[pos2 - 1] !== key) ||
      (pass[pos1 - 1] !== key && pass[pos2 - 1] === key)
    if (isValid) {
      console.log(`${line} - ${pass[pos1 -1]} - ${pass[pos2 -1]}`)
      valid++
    }
  })
  return valid
