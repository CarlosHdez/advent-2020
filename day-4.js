...
const valHcl = (color) => hexRegex.test(color)

const eyeColors = new Set(['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'])
const valEcl = (color) => eyeColors.has(color)

const idRegex = /^\d{9}$/
const validId = (id) => idRegex.test(id)

const validateBatch = (batch) => {
  let valid = 0
  const entries = batch.split('\n\n')
  entries.forEach((entry, idx) => {
    const map = entry.split(/\s/).reduce((acc, val) => {
      const [key, value] = val.split(':')
      return {...acc, [key]:value}
    }, {})
    if (!valYear(map.byr, 1920, 2002)) {
      return
    }
    if (!valYear(map.iyr, 2010, 2020)) {
      return
    }
    if (!valYear(map.eyr, 2020, 2030)) {
      return
    }
    if (!valHeight(map.hgt)) {
      return
    }
    if (!valHcl(map.hcl)) {
      return
...
console.log(validateBatch(input))
