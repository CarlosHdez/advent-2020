...
const partOne = (input) => {
  ...
  const yesPerGroup = groups.reduce((acc, answers) => {
    const uniques = new Set(answers.split(/\s/).join('').split(''))
    return acc + uniques.size
  }, 0)
  console.log(yesPerGroup)
}

const partTwo = (input) => {
  const groups = input.split('\n\n')
  const yesPerGroup = groups.reduce((acc, answers) => {
    const perUser = answers.split(/\s/)
    const uniques = perUser.slice(1).reduce((acc, singleAns) => {
      const inter = singleAns.split('')
      // if (acc.length === 0) {
      //   return inter
      // }
      return acc.filter(v => inter.includes(v))
    }, perUser[0].split(''))
    return acc + uniques.length
  }, 0)
  console.log(yesPerGroup)
}
