...
const ruleRegex = /(\d+) (.*) bags?/
const parseRule = (rule) => {
  const [outer, rest] = rule.split(' bags contain ')
  if (rest.startsWith('no other')) {
    return {[outer]: {}}
  }
  const options = rest.split(', ').reduce((acc, text) => {
    const [_, val, bag] = text.match(ruleRegex)
    acc[bag] = Number(val)
    return acc
  }, {})
  return {[outer]: options}
}

const rulesMap = input.reduce((acc, rule) => ({...acc, ...parseRule(rule)}), {})
const findBagOptions = (color) => {
  let nested = []
  Object.entries(rulesMap).forEach(([key, values]) => {
    if (values[color]) {
      nested.push(key)
    }
  })
  nested.forEach((opt) => {
    const nestedVals = findBagOptions(opt)
    nested.push(...nestedVals.nested)
  })

  return {
    viable: new Set(nested).size,
    nested
  }
}

const containedBags = (color) => {
  let total = 0
  const insideBags = Object.entries(rulesMap[color])
  insideBags.forEach(([key, bags]) => {
    const inside = containedBags(key)
    total = total + bags + (bags * inside)
  })
  return total
}

console.log(findBagOptions('shiny gold').viable)
